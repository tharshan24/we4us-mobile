...
function RequestCreationSetTwoNgo(props) {
  ...
  const getRequestedItems = async () => {
    try {
      const jsonValue = await AsyncStorage.getItem('@requestedItemsNgo');
  ...
  const getData = async () => {
    try {
      const jsonValue = await AsyncStorage.getItem('@requestInputSetOneNgo');
      const values = JSON.parse(jsonValue);
      if (values !== null) {
  ...
  const currentLocation = () => {
    navigation.navigate('findLocationMapRequestNgo', {location});
  };
  ...
  const removeAllInputsRequest = async () => {
    const keys = [
      '@requestInputSetOneNgo',
      '@selectedLocationRequestNgo',
      '@requestCreationImageNgo',
      '@requestedItemsNgo',
    ];
    try {
  ...
  const getDataSelectedLocation = async () => {
    try {
      const jsonValue = await AsyncStorage.getItem('@selectedLocationRequestNgo');
      if (jsonValue != null) {
        ...
        <View style={styles.mainContainer}>
          ...
          <View style={styles.addItems}>
            {formValues[0].name === '' || formValues[0].quantity === '' ? (
              <Button
                mode="contained"
                onPress={() => navigation.navigate('AddItemsRequestNgo')}
                style={{
                  height: 43,
                  alignItems: 'center',
                  justifyContent: 'center',
...
export default RequestCreationSetTwoNgo;
