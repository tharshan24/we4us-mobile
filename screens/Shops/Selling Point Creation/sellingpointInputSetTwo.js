...
const sellingpointInputSetTwoNgo = ({route}) => {
  ...
  const validateFieldsTwo = () => {
    // navigation.navigate('availabilityInputSetThree');
     if (madeTime === 'HH:MM:SS') {
      ...
      navigation.navigate('sellingpointInputSetThree');
    }
  };
  ...
  const storeData = async (value) => {
    try {
      ...
      await AsyncStorage.setItem('@inputSetTwoSel', jsonValue);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const getDataOne = async () => {
    try {
      const value = await AsyncStorage.getItem('@inputSetOneSel');
...
export default sellingpointInputSetTwoNgo;
