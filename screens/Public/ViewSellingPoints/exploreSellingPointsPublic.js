...
function exploreSellingPointsPublic({route}) {
  ...
  const getSellingPoints = async () => {
    try {
      await axios
        .get(constants.BASE_URL + 'org/getSellingPoints', {
          headers: {
      ...
      {loading ? (
        ...
        data.map((values) => (
          <View key={values.id} style={styles.mainContainer}>
            <TouchableOpacity
              onPress={() =>
                navigation.navigate('browseSellingPointsPublic', {
                  selling_point: values.id,
                })
              }>
...
export default exploreSellingPointsPublic;
